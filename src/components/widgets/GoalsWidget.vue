...
// UPDATED: Return Verve Theme Colors
const getProgressBarColor = (goal: Goal) => {
  if (goal.reached) return 'bg-verve-neon';   // Success = Neon Green
  if (goal.progress > 0) return 'bg-verve-orange'; // Active = Orange
  return 'bg-verve-medium'; // Empty = Medium Green
};
</script>
  ...
  <div
    ...
    <div class="flex justify-between items-center mb-4">
      ...
      <button @click="showCreateModal = true"
        class="text-sm text-verve-orange hover:text-verve-brown font-bold transition-colors">
        + Add Goal
      </button>
    </div>
    ...
    <div class="flex border-b border-verve-medium/30 mb-4">
      <button v-for="tab in ['weekly', 'monthly', 'yearly'] as const" :key="tab" @click="activeTab = tab"
        class="pb-2 px-3 text-sm font-bold transition-colors relative capitalize"
        :class="activeTab === tab ? 'text-verve-brown' : 'text-verve-brown/50 hover:text-verve-brown/80'">
        {{ tab }}
    ...
    <!-- Content -->
    <div v-if="goalStore.isLoading" class="flex-grow flex items-center justify-center text-verve-brown/60 text-sm">
      Loading...
    </div>

    <div v-else-if="currentGoals.length === 0"
      class="flex-grow flex flex-col items-center justify-center text-verve-brown/50 py-6">
    ...
    <div v-else class="space-y-5">
      <div v-for="goal in currentGoals" :key="goal.id">
        ...
        <div class="flex justify-between text-sm mb-1.5">
          <span class="font-bold text-verve-brown truncate pr-2 w-2/3" :title="goal.name">{{ goal.name }}</span>
        ...
        <div v-if="goal.type === 'manual'" class="flex items-center justify-end space-x-1">
          <button @click="handleManualUpdate(goal, false)"
            class="w-6 h-6 flex items-center justify-center rounded-lg bg-verve-light text-verve-brown hover:bg-red-100 hover:text-red-600 transition-colors"
            title="Decrease">
            -
          </button>
          ...
          <button @click="handleManualUpdate(goal, true)"
            class="w-6 h-6 flex items-center justify-center rounded-lg bg-verve-light text-verve-brown hover:bg-green-100 hover:text-green-600 transition-colors"
            title="Increase">
            +
          </button>
        </div>
