...
<template>
  <div class="p-4 sm:p-6 lg:p-8 bg-verve-light/30 min-h-[calc(100vh-64px)]">
    <div v-if="isLoading" class="text-center text-verve-brown/60 py-12">Loading activity data...</div>
    <div v-else-if="error" class="p-4 bg-red-50 text-red-600 rounded-xl border border-red-100">Error: {{ error }}</div>
    ...
    <div v-else-if="activity" class="max-w-7xl mx-auto space-y-8">
      ...
      <div class="bg-white p-6 rounded-xl shadow-sm border border-verve-medium/30">
        <div class="flex justify-between items-start mb-6">
          <div>
            <h1 class="text-3xl font-bold text-verve-brown">{{ activity.name || new
              Date(activity.start).toLocaleDateString() }}</h1>
            <p v-if="activity.name" class="text-sm text-verve-brown/60 mt-1">{{ new
              Date(activity.start).toLocaleString() }}</p>
          </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 text-center">
          ...
          <div class="p-4 bg-verve-light/20 rounded-lg">
            ...
            <p class="text-xs font-bold text-verve-brown/50 uppercase tracking-wider mt-1">Duration</p>
          </div>
          <div class="p-4 bg-verve-light/20 rounded-lg">
            <p class="text-2xl font-bold text-verve-brown">{{ activity.elevationGain?.toFixed(0) ?? '-' }}</p>
            <p class="text-xs font-bold text-verve-brown/50 uppercase tracking-wider mt-1">m Gain</p>
          </div>
          <div class="p-4 bg-verve-light/20 rounded-lg">
            <p class="text-2xl font-bold text-verve-brown">{{ activity.avg_speed?.toFixed(1) ?? '-' }}</p>
            <p class="text-xs font-bold text-verve-brown/50 uppercase tracking-wider mt-1">Avg Speed</p>
          </div>
          <div class="p-4 bg-verve-light/20 rounded-lg">
            <p class="text-2xl font-bold text-verve-brown">{{ activity.max_speed?.toFixed(1) ?? '-' }}</p>
            <p class="text-xs font-bold text-verve-brown/50 uppercase tracking-wider mt-1">Max Speed</p>
          </div>
        </div>
      </div>
