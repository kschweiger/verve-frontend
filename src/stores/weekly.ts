...
export const useWeeklyStore = defineStore('weekly', () => {
  ...
  async function fetchWeeklyStats(year: number | null, week: number | null, activityTypeId: number) {
    ...
    try {
      ...
      // The API handles null year/week to mean 'current week'
      if (year) params.append('year', year.toString());
      if (week) params.append('week', week.toString());
      params.append('activity_type_id', activityTypeId.toString());

      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/statistics/week?${params.toString()}`, {
